...
enum strategy { latency, resource };
 ...
 typename std::enable_if<N != 1 && N != 2, T>::type reduce(const T* x, Op op) {
 ...
 template<class T, int N, class Op>
 typename std::enable_if<N == 2, T>::type reduce(const T* x, Op op) {
   return op(x[0], x[1]);
 }

 template<class T, int N, class Op>
 typename std::enable_if<N == 1, T>::type reduce(const T* x, Op op) {
   return x[0];
 }
